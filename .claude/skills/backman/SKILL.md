---
name: backman
description: Use for any task involving Backman, Develeap's internal product backlog tool (backman.develeap.com). Trigger on mentions of "backman", "the backlog", "next version" / "scheduled features", "feature suggestions", "refused features", or product-version tags in a Develeap context. Provides the REST surface (products → features, versions), status flow (suggestion → backlog → scheduled → done; or refused), auth, and ready-to-run curl recipes.
version: 1.0.0
...
This skill is for programmatic access via the public REST API at `https://backman.develeap.com/api/v1`. The web UI lives at the same host.
...
Base: `https://backman.develeap.com/api/v1`. Replace `$P` with product id, `$F` with feature id, `$V` with version id.
...
B="https://backman.develeap.com/api/v1/products/$BACKMAN_PRODUCT_ID"
